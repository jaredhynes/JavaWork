...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        Map<String, Integer> neuronName = new HashMap<>();
        Map<Integer, String> neuronNameReversed = new HashMap<>();
        for (int i = 0; i<numOfNeurons; i++) {
            String name = StdIn.readString();
            neuronName.put(name, i);
            neuronNameReversed.put(i, name);
        }

        int numOfSynapses = StdIn.readInt();
        int[][] adjacencyMatrix = new int[numOfNeurons][numOfNeurons];
        for (int i = 0; i<numOfSynapses; i++) {
            String start = StdIn.readString();
            String stop = StdIn.readString();
            int startingIndex = neuronName.get(start);
            int stoppingIndex = neuronName.get(stop);
            adjacencyMatrix[startingIndex][stoppingIndex] = 1;
        }

        // Locate the neuron that has no outgoing synapses. 
        int indexOfNoOutgoing = -1;
        for (int i = 0; i < numOfNeurons; i++) {
            int sum = 0;
        ...
        for (int i  = 0; i<numOfNeurons; i++) {
            if (adjacencyMatrix[i][indexOfNoOutgoing] == 1) {
                StdOut.println(neuronNameReversed.get(i));
            }
        }
